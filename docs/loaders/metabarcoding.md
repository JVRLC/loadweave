...
Charge les données de séquençage (OTU tables) depuis SharePoint vers PostgreSQL. Source principale : IGATech (amplicons ITS).
...
flowchart TD
    ...
    IS_AMA & NOT_AMA & IS_PATH & NOT_PATH & ABREL & META --> DB[("raw.metag_batch\nraw.metag_sample\nraw.metag_otu_abundance")]
```

## Pipeline 2 passes

Le fichier OTU est lu en deux passes pour éviter de charger la matrice entière en mémoire :

```mermaid
flowchart TD
    FILE["Fichier OTU .tsv\n~16000 lignes × N colonnes"]

    FILE -->|"Passe 1 — total reads par échantillon"| PASS1
    subgraph PASS1["Passe 1"]
        direction LR
        S1["SEQ021 = 45 230 séq."]
        S2["SEQ022 = 38 910 séq."]
    end

    FILE -->|"Passe 2 — chunks de 2000 OTUs"| PARSE["Découpe taxonomie QIIME"]
...
flowchart LR
    ...
    RAW --> K["taxon_kingdom: Fungi"]
    RAW --> P["taxon_phylum: Glomeromycota"]
    RAW --> G["taxon_genus: Rhizophagus"]
    RAW --> S["taxon_species: irregularis"]
...
- **Incrémental** : `batch_already_loaded()` skip les batches déjà présents en base
- **16S skippés** : dossiers contenant `_16s_` ou `16sonly` sont ignorés
