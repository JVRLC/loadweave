...
def test_pipeline_streams_and_reports_counts(tmp_path):
    ...
    pipeline = Pipeline(
        Items(),
        [SelectFields(["first", "city"]), RenameFields({"first": "name"})],
        JsonlSink(str(output)),
    )
    result = pipeline.run()
...
def test_pipeline_reports_filtered_records(tmp_path):
    output = tmp_path / "result.jsonl"
    result = Pipeline(Items(), [FilterAll()], JsonlSink(str(output))).run()
    assert (result.extracted, result.loaded) == (2, 0)
